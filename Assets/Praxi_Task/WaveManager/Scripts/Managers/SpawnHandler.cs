...
namespace WM
{
    public class SpawnHandler : MonoBehaviour
    {
        ...
        List<GameObject> livingEnemies = new List<GameObject>();
         List<GameObject> diedMeleeEnemies = new List<GameObject>();
        List<GameObject> diedRangedEnemies = new List<GameObject>();
        ...
        public void SpawnWave(StageWaves wavesData, int enemyCount)
        {
            MeleeEnemy.Clear();
            RangedEnemy.Clear();
            ...
            float randomMeleePercent = UnityEngine.Random.Range(0f, 100f);
            int meleeCount = Mathf.RoundToInt(enemyCount * randomMeleePercent / 100f);
            int rangedCount = enemyCount - meleeCount;

            int enemiesPerPoint = enemyCount / spawnPoints.Count;
            int remainder = enemyCount % spawnPoints.Count;
            ...
            for (int i = 0; i < spawnPoints.Count; i++)
            {
                ...
                int meleeForGroup = Mathf.Min(
                    Mathf.RoundToInt(countForThisPoint * randomMeleePercent / 100f),
                    meleeLeft
                );
